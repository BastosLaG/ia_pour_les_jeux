...
double eval_heuristique_le_rat_discret_proche_de_l_homme(bt_move_t _m) {
...
bt_move_t get_heuristique_move(){
  ...
  int r_temp = 0;
  B.update_moves();
  double h_max = eval_heuristique_le_rat_discret_proche_de_l_homme(B.moves[0]);
  ...
  for (i = 0; i < B.nb_moves; i++){
    r_temp = eval_heuristique_le_rat_discret_proche_de_l_homme(B.moves[i]);
    if (h_max < r_temp) {
      r = i; 
      h_max = r_temp;
    }
