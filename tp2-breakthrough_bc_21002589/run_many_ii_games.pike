...
// pike run_many_ii_games.pike -f ./rand_player -s ./rand_player -v 1 -p 1 -l 5 -c 3 -i 0 -m 0 -d 0 

// ii stands for incomplete information
...
//  "  -d, --iinfo=IINFO_ID            0 - dark, 1 - blind.\n"

#define DUMP_GTP_PIPES		0

// communication protocol for a ii breakthrough game (as seen by the referee)
// !!! incomplete information game mode !!!
// the only difference with the classical game is that facing opponent's pawns are seen
// before sending get_genmove, referee communicates what is seen with set_board command
// and the set_play command is then useless 
...
// [referee ---> j0 ] set_newgame 5 4
// [j0 ---> referee ] =
// [referee ---> j1 ] set_newgame 5 4
// [j1 ---> referee ] =
// ...
// [if dark mode]
// [referee ---> j0 ] mode drk
...
// [j1 ---> referee ] =
// ...
// [referee ---> j0 ] set_board 0 ????????....oooooooo
// [j0 ---> referee ] =
// [referee ---> j0 ] get_genmove
// [j0 ---> referee ] = MOVE0
// [referee plays MOVE0 on its board]
// [declares j1's win in case of illegal move from j0]
// [referee ---> j1 ] set_board 1 @@@@@@@@o...????????
// [j1 ---> referee ] =
// [referee ---> j1 ] get_genmove
// [j1 ---> referee ] = MOVE1
// [referee plays MOVE0 on its board]
...
class btp_server { // breakthrough text protocol server
  ...
  void set_board(int _turn, string _boardstr) {
    send_command("setboard "+_turn+" "+_boardstr);
  }
  string get_extra() {
...
class btp_game {
  ...
  public string board_view; // same as board, used to set what is visible 
  ...
  void create(string command_line_player0, string command_line_player1,
        int _init_seed, int _mode_id, int _iinfo_id,
  ...
  void make_board_view() {
    if(iinfo_id == 0) {
      make_dark_board_view();
    } else if(iinfo_id == 1) {
      make_blind_board_view();
    }
  }
  void make_dark_board_view() {
    for(int i = 0; i < board_nbl*board_nbc; i++) {
      board_view[i] = '?';
    }
    int dec = board_nbc;
    ...
    for(int i = 0; i < board_nbl; i++) {
      for(int j = 0; j < board_nbc; j++) {
        if(board[where] == side) {
          board_view[where] = board[where];
          if(j == 0) {
            board_view[where+dec] = board[where+dec];
            board_view[where+dec+1] = board[where+dec+1];
          } else if(j == (board_nbc-1)) {
            board_view[where+dec] = board[where+dec];
            board_view[where+dec-1] = board[where+dec-1];
          } else {
            board_view[where+dec] = board[where+dec];
  ...
  void print_board_view() {
    for(int i = 0; i < board_nbl; i++) {
      werror(""+(board_nbl-i)+" ");
      for(int j = 0; j < board_nbc; j++) {
        werror("%c ",board_view[i*board_nbc+j]);
      }
      werror("\n");
    }
    werror("  ");
  ...
  void play() {
    ...
    string p1_move = "";
    while(true) {
      if(verbose >= 1) print_turn();
      array(int) Ti = System.gettimeofday();
      if(verbose >= 2) werror("calling set_board\n");
      make_board_view();
      p0->set_board(nb_turn, board_view);
      ...
      if(verbose >= 2) print_turn();
      Ti = System.gettimeofday();
      if(verbose >= 2) werror("calling set_board\n");
      make_board_view();
      p1->set_board(nb_turn, board_view);
...
int main(int argc, array(string) argv) {
  ...
  int iinfo_id = (int) Getopt.find_option(argv, "d", "iinfo", UNDEFINED, "0");
  ...
  btp_game game = btp_game(str_p0, str_p1, init_seed, mode_id, iinfo_id, game_nbl, game_nbc, str_output_dir, in_game_pause, verbose);
  if (game) {
